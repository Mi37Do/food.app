<template>
    <div v-if="salad" class="w-72 xs:w-full md:w-72 h-fit bg-white rounded-2xl relative p-5 xs:p-3 md:p-6 hover:shadow-xl hover:outline hover:outline-1 outline-primary/40 shadow-accent/5  transition-all duration-700 cursor-pointer">
        <div class="w-28 md:w-48 h-20 md:h-28 absolute -top-10 md:-top-12">
            <img :src="salad.thumbnail" class="h-full md:w-auto md:h-full object-fill">
        </div>
                
                <img v-if="salad.new" src="@/assets/new_1.png" class="w-6 absolute -top-3 md:-top-8 left-5 md:left-12" alt="">

                <div class="w-full h-full flex flex-col gap-4">
                    <div class="w-full flex justify-end items-center gap-2 text-primary">
                        <StarIcon class="w-4 h-4 fill-primary"/>
                        <span class="font-medium">{{salad.rating}} / 5</span>
                    </div>

                    <div class="w-full h-fit flex flex-col gap-2 mt-3">
                        <span class="capitalize text-base md:text-lg font-semibold truncate">{{salad.name}}</span>
                        <span class="h-fit xs:h-12 overflow-hidden text-xs text-slate-500">{{salad.storage}}</span>
                    </div>
                    ...
                    <div class="w-full h-10 flex flex-row xs:flex-col gap-2 md:flex-row items-center justify-between">
                        <div class="w-fit xs:w-full md:w-fit flex-col xs:flex-row flex md:flex-col justify-between text-base">
                            <span class="overflow-hidden text-xs text-slate-500">{{salad.price}} DZD</span>
                            <span class="overflow-hidden text-xs text-slate-500"> per {{salad.weight}} Gr.</span>
                        </div>
                    ...
                    <div class="w-full flex items-center justify-between xs:mt-4 md:mt-0 mt-0">
                        ...
                        <div class="h-8 flex flex-col justify-center items-end gap-0">
                            ...
                            <span class="md:text-xl font-bold">{{ salad.price * units}} DZD</span>
                        </div>
                    </div>
                </div>
...
const addToCart = () => {
    ...
    orderItem.id = props.salad.id
    orderItem.name = props.salad.name
    orderItem.quantity = units.value

    orderItem.price = props.salad.price
    orderItem.thumbnail = props.salad.thumbnail
